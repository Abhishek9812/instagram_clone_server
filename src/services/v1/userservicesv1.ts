...
import { createPostModel, ICreatePost } from "../../models/post/post.model";
...
class UserServicesV1 {
    ...
    public createPost = async (req: ICreatePost) => {
        try {
            const { images, author } = req;
            const imagArray = Object.values(images);

            let model = new createPostModel();
            model.images = imagArray;
            model.author = author;
            let data = await model.save();
            return { code: 200, data, msg: "Post Created Successfully" };
    ...
    public likePost = async (req: any) => {
        try {
            const { userData, id } = req;
            await createPostModel.updateOne({ _id: id }, {
                $push: { likedUsers: userData },
                $inc: { likeCount: 1 }
            }
            );
            return { code: 200, msg: "Post Liked Successfully" };
    ...
    public dislikePost = async (req: any) => {
        try {
            const { userData, id } = req;
            await createPostModel.updateOne({ _id: id }, {
                $pull: { likedUsers: { userId : userData.userId } },
                $inc: { likeCount: -1 }
            });
            return { code: 200, msg: "Post Liked Successfully" };
    ...
    public getAllPosts = async (req: ICreatePost) => {
        try {
            let data = await createPostModel.find({}).sort({ createdAt: -1 });
            return { code: 200, data, msg: "Successfull" };
